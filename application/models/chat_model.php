...
Class chat_model extends CI_Model
{
	...
	{
	  $baseurl=base_url();
	  $q=$this->db->query("select uu.user_id,up.user_name,case when up.profile_picture is null or char_length(up.profile_picture)=0 then '' else concat('$baseurl',up.profile_picture) end as profile_picture, uu.OnStatus from users uu inner join user_profile up on uu.user_id=up.user_id where uu.Deleted=0 and uu.Status=1 and uu.user_id!='$id' order by uu.user_id=1 desc,uu.OnStatus limit $start,20");
	if($q->num_rows()>0)
		return $q->result();
	else
